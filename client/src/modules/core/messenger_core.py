...
class SecureMessenger:
    ...
    def load_keys(
        self, kem_public: str, sign_public: str, data: dict, password: str
    ) -> bool:
        ...
        kem_private_key = aesgcm.decrypt(
            nonce, base64.b64decode(encrypted_keys["kem.private"]), None
        ).decode()
    ...
    def decrypt_message(self, messages: List[dict]) -> List[dict]:
        ...
        for msg in tqdm(messages):
            ...
            if (
                sha256(
                    bytes.fromhex(msg["sender_public_key"])
                    + bytes.fromhex(sender["signature_public_key"])
                ).hexdigest()
                != msg["hash_public"]
            ):
                print(
            ...
            if not self.__signature.verify(
                data, msg["signature"], sender["signature_public_key"]
            ):
                data = b"**Unverify**"
